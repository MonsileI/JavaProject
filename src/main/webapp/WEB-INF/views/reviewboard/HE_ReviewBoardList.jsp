         ...
         <div class="col-8">
            ...
            <div class="row mt-5"><!-- 배너 -->
               <div class="col">
				...
				<form action="./HE_ReviewBoardList.do" method="get">
               ...
               <div class="col" style="text-align:right;">
                  <input type="submit" value="검색" class="btn" style="width:100%;background-color:#004ba0;border-color:#004ba0;color:#ffffff">
					...
					<c:forEach items="${contentList }" var="data">
  					...
  					<div class="col">
    				<div class="thumbnail"  >
		                      글 번호 : ${data.boardVo.reviewboard_no }<br>
    				<c:choose>
		      		...
		      		<div class="caption">
		        <p><a href="./readHE_ReviewPage.do?reviewboard_no=${data.boardVo.reviewboard_no }">${data.boardVo.title }</a></p>
            ...
            <div class="row mt-2"><!-- 페이징 및 글쓰기 버튼 -->
                       ...
                       <ul class="pagination mb-0 pagination my">
                         ...
                         <c:choose>
                             <c:when test="${beginPage <= 1 }">
                                <li class="page-item disabled"><a class="page-link" href="./HE_ReviewBoardList.do?page_number=${beginPage-1 }${addParam}">&lt;</a></li>
                             </c:when>
                             <c:otherwise>
                                <li class="page-item"><a class="page-link" href="./HE_ReviewBoardList.do?page_number=${beginPage-1 }${addParam}">&lt;</a></li> <!-- 비긴 페이지 ex)6에서 1빼줘야지 -->
                             </c:otherwise>
                          </c:choose>
                         ...
                         <c:forEach begin="${beginPage }" end="${endPage }" var = "i"> <!-- begin end 반복문임!!!! 여기서 반복 설정 -->
                               <c:choose>
                                  <c:when test="${currentPage == i }">
                                     <li class="page-item active"><a class="page-link" href="./HE_ReviewBoardList.do?page_number=${i }${addParam}">${i }</a></li> <!-- 현재 클릭한 페이지 -->   
                                  </c:when>
                                  <c:otherwise>
                                     <li class="page-item"><a class="page-link" href="./HE_ReviewBoardList.do?page_number=${i }${addParam}">${i }</a></li>
                                  </c:otherwise>
                               </c:choose>
                         ...
                         <c:choose>
                             <c:when test="${endPage >= totalPageCount }">
                                <li class="page-item disabled"><a class="page-link" href="./HE_ReviewBoardList.do?page_number=${endPage +1 }${addParam}">&gt;</a></li><!-- 엔드 페이지ex)10에서 1더해줘야지 -->
                             </c:when>
                             <c:otherwise>
                                <li class="page-item"><a class="page-link" href="./HE_ReviewBoardList.do?page_number=${endPage+1 }${addParam}">&gt;</a></li>
                             </c:otherwise>
                          </c:choose>
                       </ul>
                  </nav>
               ...
               <div class="col-2 d-grid">
                  ...
                  <a class = "btn" style="background-color:#004ba0;border-color:#004ba0;color:#ffffff" href="./writeHE_ReviewPage.do">글쓰기</a> <!-- d-grid : 공간 꽉꽉 채우는거(보통 버튼 만들땐 이거 쓰자) -->
                  </c:if>
               </div>
            </div>
