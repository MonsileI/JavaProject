...
<script>
	...
	var reviewboard_no =  ${content.boardVo.reviewboard_no};
...
function processRecommend(){
		...
		xmlhttp.send("reviewboard_no="+reviewboard_no);
	}
	...
	function refreshHeart(){
		...
		xmlhttp.open("get" , "./getMyRecommendCount.do?reviewboard_no="+reviewboard_no);
		xmlhttp.send();
...
function refreshTotalCount(){
		...
		//post 방식으로 파라미터 보내는법...
		xmlhttp.open("get" , "./getTotalRecommendCount.do?reviewboard_no="+reviewboard_no);
		xmlhttp.send();
		...
		<div class="row">
			...
			<div class="col-8">
				<div class="row mt-5">
					...
					<h2 class="bg" style="width:100%;background-color:#004ba0;border-color:#004ba0;color:#ffffff">제목 : ${content.boardVo.title }
					...
					<span id="totalCountBox"></span>
						<div class="row mb-1">
					<br>
							<div class="col-8"><a href="./HE_ReviewBoardList.do">목록으로 </a></div>
							<c:if test="${!empty sessionUser  && sessionUser.member_no == content.memberVo.member_no }">
							<div class="col-2"></div>
							 <!-- boardVo_member_no써도 상관없음   -->
							<div class="col-1"><a href="./deleteContentProcess.do?reviewboard_no=${content.boardVo.reviewboard_no }">삭제</a></div> <!-- 얘는 걍 삭제만 되면 됨 -->
							<div class="col-1"><a href="./updateHE_ReviewPage.do?reviewboard_no=${content.boardVo.reviewboard_no }">수정</a></div><!--  얘는 뭔가 페이지가 있어야됨 -->
						</c:if>
							...
							<c:choose>
								<c:when test="${!empty sessionUser && sessionUser.member_no == comment.commentVo.member_no }">
									<div class="row mb-1 mt-1" style="text-align:right;">
										...
										<div class="col-1">	
											<form action="updateHE_ReviewBoardCommentPage.do" method="get">
