   ...
   <div style="width:2000px; margin: 100px auto;">
      <div class="row">
         ...
         <div class="col-8">
            ...
            <form action="./HE_ArticleBoardList.do" method="get">
            <div class="row mt-2"><!--  검색 -->
               ...
               <div class="row mt-5">
                       <div class="col-1">
                          ${data.boardVo.articleboard_no }
                      </div>
                      <div class="col-1">
                         [공지사항]
                      </div>
                      <div class="col">
                          <a href="./readHE_ArticlePage.do?articleboard_no=${data.boardVo.articleboard_no }">${data.boardVo.title }</a>
                      </div>
                      <div class="col-1">
                         ${data.memberVo.nick }
                      </div>
          ...
          <div class="row mt-2"><!-- 페이징 및 글쓰기 버튼 -->
                       ...
                       <ul class="pagination mb-0 pagination my">
                         ...
                         <c:choose>
                             <c:when test="${beginPage <= 1 }">
                                <li class="page-item disabled"><a class="page-link" href="./HE_ArticleBoardList.do?page_number=${beginPage-1 }${addParam}">&lt;</a></li>
                             </c:when>
                             <c:otherwise>
                                <li class="page-item"><a class="page-link" href="./HE_ArticleBoardList.do?page_number=${beginPage-1 }${addParam}">&lt;</a></li> <!-- 비긴 페이지 ex)6에서 1빼줘야지 -->
                             </c:otherwise>
                          </c:choose>
                         ...
                         <c:forEach begin="${beginPage }" end="${endPage }" var = "i"> <!-- begin end 반복문임!!!! 여기서 반복 설정 -->
                               <c:choose>
                                  <c:when test="${currentPage == i }">
                                     <li class="page-item active"><a class="page-link" href="./HE_ArticleBoardList.do?page_number=${i }${addParam}">${i }</a></li> <!-- 현재 클릭한 페이지 -->   
                                  </c:when>
                                  <c:otherwise>
                                     <li class="page-item"><a class="page-link" href="./HE_ArticleBoardList.do?page_number=${i }${addParam}">${i }</a></li>
                                  </c:otherwise>
                               </c:choose>
                         ...
                         <c:choose>
                             <c:when test="${endPage >= totalPageCount }">
                                <li class="page-item disabled"><a class="page-link" href="./HE_ArticleBoardList.do?page_number=${endPage +1 }${addParam}">&gt;</a></li><!-- 엔드 페이지ex)10에서 1더해줘야지 -->
                             </c:when>
                             <c:otherwise>
                                <li class="page-item"><a class="page-link" href="./HE_ArticleBoardList.do?page_number=${endPage+1 }${addParam}">&gt;</a></li>
                             </c:otherwise>
                          </c:choose>
                       </ul>
                  </nav>
               ...
               <div class="col-2 d-grid">
                  ...
                  <a class = "btn btn-dark" style="background-color:#004ba0;border-color:#004ba0;color:#ffffff" href="./writeHE_ArticlePage.do">글쓰기</a> <!-- d-grid : 공간 꽉꽉 채우는거(보통 버튼 만들땐 이거 쓰자) -->
                  </c:if>
               </div>
            </div>
            </form><!-- 검색 폼태그 -->
