...
<script src="../resources/js/articleBoard.js"></script>


<script>


	var articleboard_no =  ${content.boardVo.articleboard_no};
...
function processRecommend(){
		...
		xmlhttp.send("articleboard_no="+articleboard_no);
	}
	...
	function refreshHeart(){
		...
		xmlhttp.open("get" , "./getMyRecommendCount.do?articleboard_no="+articleboard_no);
		xmlhttp.send();
...
function refreshTotalCount(){
		...
		//post 방식으로 파라미터 보내는법...
		xmlhttp.open("get" , "./getTotalRecommendCount.do?articleboard_no="+articleboard_no);
		xmlhttp.send();
			...
			<div class="col-8">
				<div class="row mt-5">
					...
					<span id="totalCountBox"></span>
						<div class="row mb-1">
					<br>
							<div class="col-8"><a href="./HE_ArticleBoardList.do">목록으로 </a></div>
							<c:if test="${!empty sessionUser  && sessionUser.member_no == content.memberVo.member_no }">
							<div class="col-2"></div>
							 <!-- boardVo_member_no써도 상관없음   -->
							<div class="col-1"><a href="./deleteContentProcess.do?articleboard_no=${content.boardVo.articleboard_no }">삭제</a></div> <!-- 얘는 걍 삭제만 되면 됨 -->
							<div class="col-1"><a href="./updateHE_ArticlePage.do?articleboard_no=${content.boardVo.articleboard_no }">수정</a></div><!--  얘는 뭔가 페이지가 있어야됨 -->
						</c:if>
					...
					<c:if test="${!empty sessionUser}">
						<form action="writeCommentProcess.do">
							...
							<div class="mb-3">
								<div class="row">
									<div class="col-10">
										<input type="hidden" name="articleboard_no" value="${content.boardVo.articleboard_no }">
										<input type="text" class="form-control" name= "comment_content" value="" placeholder="댓글을 입력하세요." required>
									</div>
									<div class="col">
										<button class="btn" style="background-color:#004ba0;border-color:#004ba0;color:#ffffff">입력</button>
									</div>
								</div>
							...
							<c:choose>
								<c:when test="${!empty sessionUser && sessionUser.member_no == comment.commentVo.member_no }">
									<div class="row mb-1 mt-1" style="text-align:right;">
										...
										<div class="col-1">	
											<form action="updateHE_ArticleBoardCommentPage.do" method="get">
