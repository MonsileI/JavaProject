				...
				<div class="container">
					...
					<div class="row" >
					 	...
					 	<div class="py-3 text-left" style="text-align:center;font-size:20px;background-color:#090721;color:#FFFFFF;">임시보호자 요청 받은 내역</div>
						 	<hr style="width:100%; height:1px;background-color:rgba(0,0,0,0);">
					    </div>
						...
						<c:forEach items="${sitterAcceptList}" var="map" varStatus="sitter_numbering">
							...
							<div class="col py-5" style="background-color:#rgba(0,0,0,0);color:#090721;">
								...
								<div class="col-md-10 offset-md-1 py-2" style="background-color:#090721;color:#FFFFFF;">
									...
									<div style="margin:10px 10px 10px 10px;font-size:17px;">
										작성일 : <fmt:formatDate value="${processVo.key.writeDate}" pattern="yy년 MM월 dd일"/>
									</div>
								</div>
									...
									<c:forEach items="${processVo.value}" var="vo" varStatus="request_numbering">
										...
										<br>-->
										<div style="margin:10px 10px 10px 10px;font-size:13px;">
										요청받은 날짜 : <fmt:formatDate value="${vo.writeDate}" pattern="yy년 MM월 dd일"/>
										</div>
										<div style="margin:10px 10px 10px 10px;font-size:13px;"> 
										메시지 : 
										${vo.message}
										...
										<c:choose>
											...
											<c:when test="${vo.status eq 'Y'}">
												<input type="button" class = "btn btn-primary" id="sitterProcessStatusBtn" value="신청수락완료" 
													style="margin:10px 10px 10px 10px;font-size:13px;background-color:#090721;color:#FFFFFF;border:0px;"disabled>
											</c:when>
														...
														<td>
															<form action="sittetAccept.do" method="post">
																...
																<input type="submit" class = "btn btn-primary" id="sitterProcessStatusBtn" value="수락"
																	style="margin:10px 10px 10px 10px;font-size:13px;background-color:#090721;color:#FFFFFF;border:0px;">
															</form>
														</td>
														...
														<td>
															<form action="cancelSitterAccept.do" method="post">
																...
																<input type="submit" class = "btn btn-primary" id="CancelSitterProcessStatusBtn" 
																	style="margin:10px 10px 10px 10px;font-size:13px;background-color:#090721;color:#FFFFFF;border:0px;"value="수락거절">
															</form>	
														</td>
