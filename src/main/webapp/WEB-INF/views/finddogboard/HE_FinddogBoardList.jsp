   ...
   <div style="width:1500px; margin: 100px auto;">
      <div class="row">
         ...
         <div class="col-8">
            ...
            <form action="./HE_FinddogBoardList.do" method="get">
                 ...
                 <div class="col-4">
                    <c:choose>
                     <c:when test="${!empty data.imageVo.imageboard_url }">
                        <img src="/upload/${data.imageVo.imageboard_url }"class="img-circle" style="height: 200px; width: 200px; border-radius:30%" ><br>   
                     </c:when>
                     <c:otherwise>
                        <img src="../resources/img/8.jpeg" class="img-circle" style="height: 200px; width: 200px; border-radius:30%"><br>   
                     </c:otherwise>
                  </c:choose>
                 </div>
                 <div class="col mt-1">
                    <div class="row">
                          <h5>글번호 | ${data.boardVo.finddogboard_no }</h5>
                    </div>
                       <hr><!-- 제목을 감싸는 hr -->
                    <div class="row mt-1">
                       <div class="col">
                          <h2>제목 : <a href="./readHE_FinddogPage.do?finddogboard_no=${data.boardVo.finddogboard_no }">${data.boardVo.title }</a></h2>
                       </div>
                    </div>
                       <hr>
                    <div class="row">
                       ...
                       <div class="col">
                          작성일 |  <fmt:formatDate value="${data.boardVo.writeDate }" pattern="MM.dd" />
                       </div>
                    </div>   
                    <div class="row mt-4">
                       <div class="col">
                    ...
                    <div class="row">   
                       ...
                       <div class="col">
                          발견날짜 | <fmt:formatDate value="${data.boardVo.findDate }" pattern="MM.dd" />
                       </div>
                    </div>
                 </div>
             </div>
            ...
            <div class="row mt-2"><!-- 페이징 및 글쓰기 버튼 -->
                       ...
                       <ul class="pagination mb-0 pagination my">
                         ...
                         <c:choose>
                             <c:when test="${beginPage <= 1 }">
                                <li class="page-item disabled"><a class="page-link" href="./HE_FinddogBoardList.do?page_number=${beginPage-1 }${addParam}">&lt;</a></li>
                             </c:when>
                             <c:otherwise>
                                <li class="page-item"><a class="page-link" href="./HE_FinddogBoardList.do?page_number=${beginPage-1 }${addParam}">&lt;</a></li> <!-- 비긴 페이지 ex)6에서 1빼줘야지 -->
                             </c:otherwise>
                          </c:choose>
                         ...
                         <c:forEach begin="${beginPage }" end="${endPage }" var = "i"> <!-- begin end 반복문임!!!! 여기서 반복 설정 -->
                               <c:choose>
                                  <c:when test="${currentPage == i }">
                                     <li class="page-item active"><a class="page-link" href="./HE_FinddogBoardList.do?page_number=${i }${addParam}">${i }</a></li> <!-- 현재 클릭한 페이지 -->   
                                  </c:when>
                                  <c:otherwise>
                                     <li class="page-item"><a class="page-link" href="./HE_FinddogBoardList.do?page_number=${i }${addParam}">${i }</a></li>
                                  </c:otherwise>
                               </c:choose>
                         ...
                         <c:choose>
                             <c:when test="${endPage >= totalPageCount }">
                                <li class="page-item disabled"><a class="page-link" href="./HE_FinddogBoardList.do?page_number=${endPage +1 }${addParam}">&gt;</a></li><!-- 엔드 페이지ex)10에서 1더해줘야지 -->
                             </c:when>
                             <c:otherwise>
                                <li class="page-item"><a class="page-link" href="./HE_FinddogBoardList.do?page_number=${endPage+1 }${addParam}">&gt;</a></li>
                             </c:otherwise>
                          </c:choose>
                       </ul>
                  </nav>
               ...
               <div class="col-2 d-grid">
                  ...
                  <a class = "btn"  style="background-color:#004ba0;border-color:#004ba0;color:#ffffff" href="./writeHE_FinddogPage.do">글쓰기</a> <!-- d-grid : 공간 꽉꽉 채우는거(보통 버튼 만들땐 이거 쓰자) -->
                  </c:if>
               </div>
            </div>
         </form><!-- 검색 폼태그 -->
